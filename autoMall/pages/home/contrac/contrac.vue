	...
	<view class="tryGO">
		...
		<view class="tryGoods">
			<view class="shihuoLogo">
				<image src="https://carshop.duoka361.cn/images/static/image/home/qyLogo.png" @click="toSH()"></image>
			</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			async initRegisterGift() {
				let data = await getQY()
				this.BGimage = data.data.signing_image
			},
			async initGoods() {
				let query = {
					brand_id: '',
			...
			toSH() {
				uni.navigateTo({
					url: '/pages/store/signAgreement/signInfo'
				})
			},
		}
	}
